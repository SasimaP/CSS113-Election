...
class AuthenticatePage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        ttk.Label(self , text="Authentication" , font=("Arial", 20)).grid()
        ttk.Label(self , text="STD_ID" , font=("Arial", 12)).grid()
        self.userEntry = ttk.Entry(self )
        self.userEntry.grid(row =  1, column = 1)
        ttk.Label(self , text="PASS" , font=("Arial", 12)).grid()
        self.passEntry = ttk.Entry(self , show="*" )
        self.passEntry.grid(row = 2 , column = 1)

        ttk.Button(self ,text = "Submit", command= lambda : self.login(controller)).grid(sticky = 'NE')
...
class MenuPage(tk.Frame):
     def __init__(self, parent, controller):
        ttk.Frame.__init__(self, parent)
        ttk.Label(self , text="Menu" , font=("Arial", 20)).grid(row = 0)
